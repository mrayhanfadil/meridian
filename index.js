    ...
    for (const p of positionData) {
      ...
      if (exit) {
        if (exit.action === "TRAILING_TP" && exit.needs_confirmation && shouldUsePnlRecheck()) {
          const effectiveDrop = exit.effective_drop_pct ?? config.management.trailingDropPct;
...
export async function runScreeningCycle({ silent = false } = {}) {
  ...
  try {
    ...
    const strategyBlock = `DEPLOY STRATEGY: ${deployStrategy} (from config) | bins_above: max(8, round(bins_below / 3)) for spot strategy (AUTO-FILLED in executor if LLM passes 0) | deposit: SOL only (amount_y, amount_x=0)`
  ...
  const pnlPollInterval = setInterval(async () => {
    ...
    try {
      ...
      for (const p of result.positions) {
        ...
        const decayCheck = checkYieldDecay(p.position, p, config.management);
        if (decayCheck) {
          const cooldownMs = config.schedule.managementIntervalMin * 60 * 1000;
          const sinceLastTrigger = Date.now() - _pollTriggeredAt;
          if (sinceLastTrigger >= cooldownMs) {
            _pollTriggeredAt = Date.now();
            log("state", `[PnL poll] ${decayCheck.action}: ${p.pair} — ${decayCheck.reason} — triggering management`);
            runManagementCycle({ silent: true }).catch((e) => log("cron_error", `Poll-triggered management failed: ${e.message}`));
          } else {
            log("state", `[PnL poll] ${decayCheck.action}: ${p.pair} — ${decayCheck.reason} — cooldown (${Math.round((cooldownMs - sinceLastTrigger) / 1000)}s left)`);
          }
          break;
        }
      }
